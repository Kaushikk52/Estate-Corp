...
const NavLink = ({ href, children }: { href: string; children: React.ReactNode }) => (
  <a href={href} className="group relative text-base font-semibold text-gray-800 hover:text-gray-900">
    {children}
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center space-x-6">
            ...
            <div className="relative group">
              <button
                className="group relative text-base font-semibold text-gray-800 hover:text-gray-900 focus:outline-none"
              >
                Residential
                <ChevronDown size={16} className="ml-1 inline-block transition-transform duration-200 group-hover:rotate-180" />
                <span className="absolute left-0 bottom-0 h-0.5 w-0 bg-gray-800 transition-all duration-300 group-hover:w-full"></span>
              </button>
          ...
          <div className="flex items-center space-x-4">
            {/* <button className="text-gray-600 hover:text-gray-800 transition-colors duration-200" aria-label="Search">
              <Search size={20} />
            </button> */}
            <a ></a>
            <AuthPopup />
            ...
            <button className="text-gray-600 hover:text-gray-800 transition-colors duration-200" aria-label="Account">
              <a href="/dashboard"><User size={20} /></a>
            </button>
          </div>
        </div>
      </nav>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden absolute left-0 right-0 top-full bg-white shadow-lg z-40">
          <div className="py-2">
            ...
            >
              Residential
              <ChevronDown size={16} className={`transition-transform duration-200 ${isResidentialsOpen ? 'rotate-180' : ''}`} />
            </button>
            {isResidentialsOpen && (
              <div className="bg-gray-50 py-2">
                <a href="/residential/rent" className="block px-8 py-2 text-sm text-gray-700 hover:bg-gray-100 transition-colors duration-200">Rent</a>
                <a href="/residential/buy" className="block px-8 py-2 text-sm text-gray-700 hover:bg-gray-100 transition-colors duration-200">Buy</a>
              </div>
            )}
            <button
            ...
            >
              Commercial
              <ChevronDown size={16} className={`transition-transform duration-200 ${isCommercialsOpen ? 'rotate-180' : ''}`} />
            </button>
            {isCommercialsOpen && (
              <div className="bg-gray-50 py-2">
                <a href="/commercial/rent" className="block px-8 py-2 text-sm text-gray-700 hover:bg-gray-100 transition-colors duration-200">Rent</a>
                <a href="/commercial/buy" className="block px-8 py-2 text-sm text-gray-700 hover:bg-gray-100 transition-colors duration-200">Buy</a>
              </div>
            )}
            <a href="/add-property" className="block px-4 py-2 text-sm text-blue-500 hover:bg-gray-100 transition-colors duration-200">
              Add Property
              <span className="ml-1 text-xs bg-gradient-to-r from-blue-500 to-purple-500 text-white px-1 rounded">FREE</span>
            </a>
          </div>
        </div>
      )}
